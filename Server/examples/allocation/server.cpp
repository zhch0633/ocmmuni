...
template <typename Handler> inline custom_alloc_handler<Handler> make_custom_alloc_handler(handler_allocator& a, Handler h) {
...
public:
    ...
    void start() {
      socket_.async_read_some(asio::buffer(data_), make_custom_alloc_handler(allocator_,
...
int main(int argc, char* argv[])
{
  try
  {
    if (argc != 2) {
      ...
      Server s = Server(io_service,8080);
      io_service.run();
